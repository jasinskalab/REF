function E=efficiency_bin(A,local,SSlist)
...
% Remove short channel list
E_final = nan*zeros(length(A),1);

% Remove SSlist entries
chan_index = 1:1:length(A);
chan_index(SSlist) = [];
...
if exist('local','var') && local            %local efficiency
    ...
    % Correct channels index due to excluded channels SSlist
    cnt = 0;
